...
import {isCanvasSupported,getBezierPoints} from '../helpers/utils';
import {RenderHelper} from '../helpers/render';
import {AnimationHelper} from '../helpers/animator';

export var SplineChart = function(plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			var hexColor = '#4286f4';//intToHexColorString(seriesId);
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (dataPoints[i].markerSize > 0 || dataSeries.markerSize > 0) {
						...
						var markerColor = '#4286f4'//intToHexColorString(id);
